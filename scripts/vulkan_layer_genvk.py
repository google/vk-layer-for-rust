...
sys.path.insert(
    0,
    str(Path(__file__).absolute().parents[1] / "third_party" / "Vulkan-Headers" / "registry")
)  # noqa: E402
...
def make_gen_opts(
        ...
        emit_extension_pat: str) -> dict[str, tuple[OutputGenerator, GeneratorOptions]]:
    return {
        "layer_trait.rs": (
            LayerTraitGenerator,
            GeneratorOptions(conventions=conventions,
                             filename="layer_trait.rs", directory=directory, genpath=None,
                             apiname="vulkan", profile=None, versions=feature_pat,
                             emitversions=feature_pat, defaultExtensions='vulkan',
        ...
        "global_simple_intercept.rs": (
            ...
            GeneratorOptions(conventions=conventions,
                             filename="global_simple_intercept.rs", directory=directory,
                             genpath=None, apiname="vulkan", profile=None, versions=feature_pat,
                             emitversions=feature_pat, defaultExtensions='vulkan',
                             addExtensions=add_extension_pat,
                             removeExtensions=remove_extension_pat,
                             emitExtensions=emit_extension_pat)
...
def main():
    logging.basicConfig(
        format=("[%(asctime)s %(levelname)s %(module)s] %(funcName)s(%(filename)s:%(lineno)d): "
    ...
    parser.add_argument('-remove-extensions', action='append', default=[],
                        help='Specify an extension or extensions to remove from targets')
    parser.add_argument('-emit-extensions', action='append', default=[],
    ...
    gen_opts = make_gen_opts(
        conventions=VulkanConventions(), directory=args.directory,
